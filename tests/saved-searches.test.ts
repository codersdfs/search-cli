...
describe("saved-searches", () => {
  beforeEach(async () => {
    ...
    const { getSavedSearches, deleteSavedSearch } =
      await import("../src/saved-searches.ts");
  ...
  afterEach(() => {
    try {
      unlinkSync(join(testDir, "ghfind", "saved-searches.json"));
    } catch {}
  });

  it("saves and retrieves a search", async () => {
    const { saveSearch, getSavedSearches } =
      await import("../src/saved-searches.ts");
    saveSearch(
      "Rust CLI tools",
  ...
  it("deletes a saved search", async () => {
    const { saveSearch, deleteSavedSearch, getSavedSearches } =
      await import("../src/saved-searches.ts");
  ...
  it("touches lastRunAt on use", async () => {
    const { saveSearch, touchSavedSearch, getSavedSearches } =
      await import("../src/saved-searches.ts");
